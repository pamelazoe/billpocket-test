...
// My first step was to create a function that could merge the arrays
// alternating between product and price and turned them into an object.
// I applied this function to the products with the right price and the
// ones with the problematic prices to get something arrays like this:
// Products for sale: { rice: 16.89, sugar: 56.92, wheat: 20.89, cheese: 345.99 }
// Products sold: { rice: 18.99, cheese: 400.89 }
// The last step was to cross reference what products present in both objects 
// the ones on the "database". This were stored 
...
const priceCheck = (products, productPrices, productSold, soldPrice) => {
    ...
    //let errors = Object.entries(forSale).filter(([key, val]) => sold[key] !== val && key in sold).reduce((sold, [key, v]) => ({...sold, [key]: v}), {});
